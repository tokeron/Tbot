...
function doGet(e) {
  ...
  /** @type {String} */
  let password = ScriptProperties.getProperty("password");
  if(!password || (e.parameter&&e.parameter.pass&&e.parameter.pass === password))return HtmlService.createHtmlOutputFromFile("controlPanel.html");
}

function setPassword(newPass){
  ScriptProperties.setProperty("password",newPass);
}
...
function handleCallback(contents){
  ...
  var businesses = dataBaseEx.getSheetByName("busi");
  ...
  if (mode1 == "Delete A Course From My List"){//course to delete
    ...
    var maxCol = businesses.getRange(2, 2).getValue();
    var maxRow = businesses.getRange(3, 2).getValue();
    var topicBase = businesses.getRange(4, 2).getValue();
    var sectionBase = businesses.getRange(5, 2).getValue();
    var sectionsNum = businesses.getRange(6, 2).getValue();
    var topicNum = businesses.getRange(7, 2).getValue();   
    if (data == "Add a Topic \ud83c\udfea"){
      ...
      for (var i = sectionBase + 1; i < sectionBase*sectionsNum; i += sectionBase){
        var currTopic = businesses.getRange(1, i).getValue();
    ...
    */
      ...
      var currBusi = businesses.createTextFinder(data).findNext();
      if (currBusi){
    ...
    if (currBusi && busiRow == 1){//Show all business in the topic
      var courseList = [];
      var numberList = [];
      var topiCol = businesses.createTextFinder(data).findNext().getColumn();
      var counter = businesses.getRange(2, topiCol-1).getValue();
      ...
      for (var i = topicBase + 1; i < topicBase + counter + 1; ++i){
        var currBusi = businesses.getRange(i, topiCol).getValue();
      ...
      var description = businesses.getRange(busiRow, busiCol+1).getValue();
...
//        sendText(id, "Cheking..");
    ...
    var currBusi = businesses.createTextFinder(mode2).findNext();
    if (currBusi){
      var busiCol = currBusi.getColumn();
      var busiRow = currBusi.getRow();
    }
    if (mode2 == "Add a Topic \ud83c\udfea"){
      var isExist = businesses.createTextFinder(text).findNext();
      if (isExist) sendText(id, "This topic is already exists. You can add your business by 'Add a Business \ud83c\udfea' button after entering the topic");
        ...
        businesses.getRange(1, sectionBase*sectionsNum+1).setValue(text);
        businesses.getRange(2, sectionBase*sectionsNum).setValue(0);
        businesses.getRange(6, 2).setValue(sectionsNum+1);
      ...
      var textFinder = businesses.createTextFinder(mode3);
      var next = textFinder.findNext();
      if (next !== null){
        var nextRow = next.getRow();
        var nextCol = next.getColumn();
        if (text == businesses.getRange(nextRow, nextCol - 1).getValue()){ //the password is good
          var lastRow = 2 + businesses.getRange(2, nextCol-1).getValue();
          for (var i = nextCol-1; i < nextCol-1+6; i++){//move last to this row
            var temp = businesses.getRange(lastRow,i).getValue();
            businesses.getRange(nextRow,i).setValue(temp);
          }
          businesses.getRange(2, nextCol-1).setValue(lastRow-2-1);
    ...
    var currTopic = businesses.createTextFinder(topic).findNext();
    var topicCol = 0;
    var topicCounter = 0;
    //sendText(id, "curr topic: "+topic+" "+currTopic);
    if (currTopic){
      topicCol = currTopic.getColumn();
      topicCounter = businesses.getRange(2, topicCol-1).getValue(); 
    }
    ...
    if (mode2 == "Password"){
      ...
      if (text.length >= 34) sendText(id, "The name is too long. Please choose another name for your business");
        ...
        businesses.getRange(topicBase+topicCounter+1, topicCol).setValue(text);//set name
        sendText(id, text+" is initialized. Please send a password in order to be able to make changes in the future..");
        businesses.getRange(2, topicCol-1).setValue(topicCounter+1);//conter++
      ...
      businesses.getRange(topicBase+topicCounter, topicCol-1).setValue(text);//set password
...
//      }
      ...
      businesses.getRange(topicBase+topicCounter, topicCol+1).setValue(text);//set Description
...
//      }
      ...
      businesses.getRange(topicBase+topicCounter, topicCol+3).setValue(text);//set contact info
      ...
      var busiToDelete = businesses.createTextFinder(text).findNext();
      var busiRow = busiToDelete.getRow();
      var busiCol = busiToDelete.getColumn();
      var afteLastInCol = businesses.getRange(1, busiCol).getValue();
      var lastInCol = businesses.getRange(afteLastInCol-1, busiCol).getValue();
      var lastInColPass = businesses.getRange(afteLastInCol-1, busiCol-1).getValue();
      var lastInColDes = businesses.getRange(afteLastInCol-1, busiCol+1).getValue();
      var lastInColContact = businesses.getRange(afteLastInCol-1, busiCol+3).getValue();
      
      businesses.getRange(busiRow, busiCol).setValue(lastInCol);
      businesses.getRange(busiRow-1, busiCol).setValue(lastInColPass);
      businesses.getRange(busiRow+1, busiCol).setValue(lastInColDes);
      businesses.getRange(busiRow+3, busiCol).setValue(lastInColContact);
      businesses.getRange(1, busiCol).setValue(afteLastInCol-1);
    }
      
    if (text == "Location"){
      if (currBusi){
        var textToSend = businesses.getRange(busiRow, busiCol+2).getValue();
        sendText(id, textToSend);
      }
    }else if (text == "Get in Contact"){
      if (currBusi){
        var textToSend = businesses.getRange(busiRow, busiCol+3).getValue();
        sendText(id, textToSend);
      }
    }else if (text == "Prices"){
      if (currBusi){
        var textToSend = businesses.getRange(busiRow, busiCol+4).getValue();
        sendText(id, textToSend);
      }
    }
    ...
    var sectionsNum = businesses.getRange(6, 2).getValue();
    var topicNum = businesses.getRange(7, 2).getValue(); 
    var currBusi = businesses.createTextFinder(mode3).findNext();
    var busiRow = currBusi.getRow();
    var busiCol = currBusi.getColumn();
    ...
    businesses.getRange(busiRow, busiCol).setValue(text);
